"""Extracts scam URLs found at https://www.globalantiscam.org/list-of-scam-websites-and-links and writes them to a 
.txt blocklist
"""
import asyncio
import json
...
async def extract_scam_urls() -> set[str]:
    ...
    try:
        ...
        # this <link> tag's href points to a json file containing the feed URLs
        only_link_tag_with_id_features_kowmu = SoupStrainer("link",id="features_kowmu",
                                                            rel="preload", crossorigin="anonymous")
        soup = BeautifulSoup(main_page, "lxml",parse_only=only_link_tag_with_id_features_kowmu)
        tag = soup.find().get('href',"")
        
        if tag != "":
            # Find feed URLs
            content = json.loads((await get_async([tag]))[tag])
            feed_urls = [feed.replace("https:/","https://") for feed in get_recursively(content,'url')]
            ...
            # Extract scam URLs
            urls = []
            # All scam URLs are encapsulated in <span style="color:black"> tags  
            only_span_tag_with_style_colorblack = SoupStrainer("span",style="color:black")
            for feed_content in feed_contents.values():
                soup = BeautifulSoup(feed_content,"lxml",parse_only=only_span_tag_with_style_colorblack)
                urls += [extractor.find_urls(p.get_text(strip=True)) 
                        for p in soup.find_all(lambda tag: tag.string is not None)]    
            # Some lines may have multiple URLs or no valid URLs
            return set(clean_url(url) for url in flatten(urls))
        else:
            logger.error("'href' tag not found!")
            return set()
    except Exception as error:
        logger.error(error)
        return set()
